...
var KLLParser = require("./antlr/KLLParser.js");
...
// Visit a parse tree produced by KLLParser#global_statements.
...
KLLVisitor.prototype.visitLayer_block = function (ctx) {
  // if this blocks extends another we need to push this layer onto the scope of whatever it extends
  const { children } = ctx;

  const isExtension = children?.contains(
    (child) => child.ruleIndex === KLLParser.RULE_extends_statement
  );
  console.log("layer", isExtension);
  return this.visitChildren(ctx);
};
